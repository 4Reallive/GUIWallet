...
// Package turtlecoinwalletdrpcgo handles the the rpc connection between your app and service
...
// returned balances are expressed in CELC, not in 0.01 CELC
...
// RequestStatus requests service connection and sync status
...
// parameters amount and fee are expressed in CELC, not 0.01 CELC
func SendTransaction(addressRecipient string, amount float64, paymentID string, fee float64, rpcPassword string) (transactionHash string, err error) {
	...
	amountInt := uint64(amount * 100000) // expressed in hundredth of CELC
	feeInt := uint64(fee * 100000)       // expressed in hundredth of CELC
...
// threshold is the value that determines which outputs will be optimized. Only the outputs, lesser than the threshold value, will be included into a fusion transaction (threshold is expressed in CELC, not 0.01 CELC).
...
func EstimateFusion(threshold int, addresses []string, rpcPassword string) (fusionReadyCount int, totalOutputCount int, err error) {
	...
	threshold *= 100000 // expressed in hundredth of CELC
...
// returned fee is expressed in CELC, not in 0.01 CELC
