...
import (
	"Celestial-Wallet/turtlecoinwalletdrpcgo"
	"bufio"
	"io"
	"math/rand"
	"os"
...
var (
	...
	// will be set to a random string when starting service
	rpcPassword = ""

	cmdWalletd     *exec.Cmd
	cmdTurtleCoind *exec.Cmd

	// WalletdOpenAndRunning is true when service is running with a wallet open
...
func RequestAvailableBalanceToBeSpent(transferFeeString string) (availableBalance float64, err error) {
	...
	transferFee, err := strconv.ParseFloat(transferFeeString, 64) // transferFee is expressed in CELC
	if err != nil {
...
func SendTransaction(transferAddress string, transferAmountString string, transferPaymentID string, transferFeeString string) (transactionHash string, err error) {
	...
	if !strings.HasPrefix(transferAddress, "C") || (len(transferAddress) != 95 && len(transferAddress) != 183) {
	...
	transferAmount, err := strconv.ParseFloat(transferAmountString, 64) // transferAmount is expressed in CELC
	if err != nil {
		return "", errors.New("amount is invalid")
	}

	if transferAmount <= 0 {
		return "", errors.New("amount of CELC to be sent should be greater than 0")
	}
...
// SaveWallet saves the sync status of the wallet. To be done regularly so when service crashes, sync is not lost
...
// StartWalletd starts the service daemon with the set wallet info
...
// useCheckpoints is true if Celestiald should be run with "--load-checkpoints"
func StartWalletd(walletPath string, walletPassword string, useRemoteNode bool, useCheckpoints bool, daemonAddress string, daemonPort string) (err error) {
	...
	if isWalletdRunning() {
		errorMessage := "service is already running in the background.\nPlease close it via "
	...
	if isPlatformDarwin {
		...
		pathToAppLibDir := pathToHomeDir + "/Library/Application Support/Celestial Wallet"
	...
	if !useRemoteNode && !isTurtleCoindRunning() {
		...
		log.Info("Opening Celestiald and waiting for it to be ready.")
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					log.Error("Failed reading Celestiald log file line by line: ", err)
				}
			}
			if strings.Contains(line, "Imported block with index") {
				log.Info("Celestiald importing blocks: ", line)
			}
			if strings.Contains(line, "Core rpc server started ok") {
				log.Info("Celestiald ready (rpc server started ok).")
				break
			}
			if strings.Contains(line, "Node stopped.") {
				errorMessage := "Error Celestiald: 'Node stopped'"
	...
	log.Info("Opening service and waiting for it to be ready.")
	...
	for !successLaunchingWalletd && time.Duration(timesCheckLog)*timeBetweenChecks < maxWaitingTime {
		...
		for {
			...
			if strings.Contains(line, "Wallet loading is finished.") {
				...
				log.Info("service ready ('Wallet loading is finished.').")
				break
			}
			if strings.Contains(line, "Imported block with index") {
				timesCheckLog = 0
			...
			if strings.Contains(line, "INFO    Stopped") {
				...
				if len(listWalletdErrors) > 0 {
					...
					errorMessage = "service stopped with unknown error"
				}
	...
	if err != nil {
		killWalletd()
		return errors.New("error communicating with service via rpc")
	}
...
func GracefullyQuitWalletd() {
	...
	if WalletdOpenAndRunning || cmdWalletd != nil {
		...
		if isPlatformWindows {
			...
			if err != nil {
				log.Error("failed to kill service: " + err.Error())
			} else {
				log.Info("service killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdWalletd.Process.Kill(); err != nil {
					log.Warning("failed to kill service: " + err.Error())
				}
				log.Info("service killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("service finished with error: " + err.Error())
				}
				log.Info("service killed without error")
			}
		}
...
// GracefullyQuitCelestiald stops the Celestiald daemon
func GracefullyQuitTurtleCoind() {
	...
	if cmdTurtleCoind != nil {
		...
		if isPlatformWindows {
			// because syscall.SIGTERM does not work in windows. We have to kill Celestiald.

			err = cmdTurtleCoind.Process.Kill()
			if err != nil {
				log.Error("failed to kill Celestiald: " + err.Error())
			} else {
				log.Info("Celestiald killed without error")
			}
		} else {
			...
			case <-time.After(5 * time.Second):
				if err := cmdTurtleCoind.Process.Kill(); err != nil {
					log.Warning("failed to kill Celestiald: " + err.Error())
				}
				log.Info("Celestiald killed as stopping process timed out")
			case err := <-done:
				if err != nil {
					log.Warning("Celestiald finished with error: " + err.Error())
				}
				log.Info("Celestiald killed without error")
			}
		}
...
// CreateWallet calls service to create a new wallet. If privateViewKey, privateSpendKey and mnemonicSeed are empty strings, a new wallet will be generated. If they are not empty, a wallet will be generated from those keys or from the seed (import)
// walletFilename is the filename chosen by the user. The created wallet file will be located in the same folder as service.
...
func CreateWallet(walletFilename string, walletPassword string, walletPasswordConfirmation string, privateViewKey string, privateSpendKey string, mnemonicSeed string, scanHeight string) (err error) {
	...
	if WalletdOpenAndRunning {
		return errors.New("service is already running. It should be stopped before being able to generate a new wallet")
	}
